...
const TabContentStocks = () => {
    ...
    return (
        <div className="flex flex-row flex-wrap gap-3">
            {stocks.walletBalance.map((asset, i) => {
                const marketPriceData = stocks.marketPrices.find((marketAsset) => marketAsset.name === asset.name) ?? {
                    seriesPrice: [],
                };
                        ...
                        <CardHeader className="relative">
                            <CardTitle className="flex flex-row gap-4">
                                <div className="flex flex-row align-bottom gap-4">
                                    <img src={asset.iconUrl} style={{ width: "40px" }} />
                                    {asset.name}
                                </div>
                            </CardTitle>
